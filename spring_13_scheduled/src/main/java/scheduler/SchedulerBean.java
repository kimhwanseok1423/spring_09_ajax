...
/*  로그(log) 프로그램 개발이나 웅영시 문제점을 추적하거나 운영상태를 모니터링하기 위한 기능이다.
 *  System.out.println();을 사용하여 로그를 확인 할 수 있으나 이보다는 로그를 기록하는 방법을 사용한다.
 *  log4j은 다음과 같은 레벨을 가진다.
 *  TRACE > DEBUG >INFO >WARN >ERROR > FATAL
 *  만약에 info로 세팅하면 INFO,WARN,ERROR,FATAL은 기록된다.
 *  
 *   FATAL: 심각한 에러-> 프로그램 자체가 실행되지 않을 수있다.
 *   ERROR: 문제가 발생하면 소프트웨어 일부가 기능을 수행하지 못할수있다.
 *   WARNING: 소프트웨어가 동작하고 있지만, 예상지 못한 일이 발생하고 가까운 미래에 문제가 발생할 수 있다는 경고
 *               예로 디스크 공간 부족->프로ㅗ그램은 동작되지만 앞으로 문제가 발생할수 있음.
 *   
 *   INFO:작업이 정상적으로 작동하고 있음을 메시지를 통해 확인하고  싶은 경우에 사요ㅗㅇ
